...
#gc535, sl2954
#This program integrate the two wheel control with servo history
#scrolling
################################################################
...
########## static button#########
start='stop'
static_text = {(160,120):start, (40,40):'left history', (240,40):'right history', (240,180):'quit'}
    ...
    for event in pygame.event.get():
        ...
        if(event.type is pygame.MOUSEBUTTONDOWN):
            ...
            if 140<=x<=180 and 100<=y<=140 and st_re== 1:
                ...
                l_que.pop(2)
                l_que.insert(0, ('stop        '+str(int(time.time())-start_time)) )
                pre_lstatus = l_que[1]
                #print(pre_lstatus)
                servo_control(servo1, 'stop')
                ...
                r_que.pop(2)
                r_que.insert(0, ('stop        '+str(int(time.time())-start_time)) )
                pre_rstatus = r_que[1]
                #print(pre_rstatus)
                servo_control(servo2, 'stop')
                color = GREEN
            #Resume is pressed
                ...
                l_que.pop(2)
                l_que.insert(0, (pre_lstatus[:10]+str(int(time.time())-start_time)) )
                ## resume right server and update history
                r_que.pop(2)
                r_que.insert(0, (pre_rstatus[:10]+str(int(time.time())-start_time)) )
                ## recover pre_status
                if pre_lstatus[1] == 'l':
                    cmd1 = 'clockwise'
